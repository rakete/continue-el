...
(defun continue-sourcemarker-line-score-search (smarker)
  (let* ((lines-list (split-string (buffer-substring-no-properties (point-min) (point-max)) "\n" t))
         ...
         (n 0)
         (above-words-2 (cl-remove-duplicates (split-string (nth 0 (cdr (assoc :lines-above smarker))) " " t) :test 'equal))
         (above-words-1 (cl-remove-duplicates (split-string (nth 1 (cdr (assoc :lines-above smarker))) " " t) :test 'equal))
         (pivot-words (cl-remove-duplicates (split-string (cdr (assoc :lines-center smarker)) " " t) :test 'equal))
         (below-words+1 (cl-remove-duplicates (split-string (nth 0 (cdr (assoc :lines-below smarker))) " " t) :test 'equal))
         (below-words+2 (cl-remove-duplicates (split-string (nth 1 (cdr (assoc :lines-below smarker))) " " t) :test 'equal))
         (high-score 0)
         (best-line nil))
    (cl-dolist (line (append (list "#bobp#" "#bobp#") lines-list (list "#eobp#" "#eobp#")))
      (let ((words (split-string line " " t)))
        ...
        (when (>= n 4)
          (let ((score 0)
            ...
            (cl-dolist (w above-words-2)
              (when (cl-position w line-words-2 :test 'equal)
                (setq score (+ score 6))))
            (cl-dolist (w above-words-1)
              (when (cl-position w line-words-1 :test 'equal)
                (setq score (+ score 10))))
            (cl-dolist (w pivot-words)
              (when (cl-position w line-words-0 :test 'equal)
                (setq score (+ score 12))))
            (cl-dolist (w below-words+1)
              (when (cl-position w line-words+1 :test 'equal)
                (setq score (+ score 10))))
            (cl-dolist (w below-words+2)
              (when (cl-position w line-words+2 :test 'equal)
                (setq score (+ score 6))))
                     ...
                     (cl-block "match-testing-loop"
                       (let (tm done-first)
                         (while (setq tm (or (unless done-first
                           ...
                           (let ((tm-score 1))
                             (cl-dolist (against-token (cdr matching-order))
